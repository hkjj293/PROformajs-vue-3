...
<template>
  <div>
    ...
    <div v-show="!dataDefinitionPath && !sourcePath && !candidatePath">
      <h4>
        <pc-icon :icon="task.constructor.name" :scale="2" class="mb-1" /> {{ task.name }}
      </h4>
        ...
        <li class="nav-item" role="presentation" v-if="task.constructor.name == 'Decision'">
          <button :class="'nav-link ' + (tabIndex == 3 ? 'active' : '')" :id="'pc-task-tabs-candidates'"
            ...
            <span class="d-none d-sm-block">
              Candidates <span class="badge rounded-pill text-bg-secondary"
                v-if="task.candidates && task.candidates.length > 0">
                {{ task.candidates.length }}
              </span>
            </span>
          ...
          <ul class="list-unstyled">
            <li v-for="(issue, idx) in issues" :key="idx"
              ...
              <span v-if="issue.path != path" class="clickable text-primary" @click="selectTask(issue.path)">{{
                localPath(issue) }}:</span> {{ issue.msg }}<span
              ...
              <tbody>
                <tr v-for="(def, idx) in dataDefinitions" :key="idx">
                  ...
                  <td class="clickable" @click="selectDef(idx)">
                    <span class="badge rounded-pill text-bg-secondary">{{ def.constructor.name }}</span>
        ...
        <div :class="'tab-pane ' + (tabIndex == 2 ? 'active' : '')" id="pc-task-tabs-source-p"
          ...
          <form>
            <template v-if="sourceDataDefs().length > 0">
                ...
                <div class="form-row">
                  <div class="col">
                    <select class="form-control form-control-sm" ref="newSourceType">
                      <option v-for="dd in availableDataDefs()" :key="dd.path()">{{ dd.name }}</option>
                    </select>
                  </div>
                  <div class="col">
              ...
              <template v-else>
                No more available data definitions
              </template>
            ...
            <p v-else>
              You need to add at least one manual data definition to the root plan before being able to source it in this
              enquiry.
            </p>
          </form>
        </div>
                ...
                <tr v-for="(cand, idx) in task.candidates" :key="idx">
                  ...
                  <td class="clickable" @click="selectCandidate(idx)">
                    <span class="badge rounded-pill text-bg-secondary" pill>
                      {{ cand.arguments.length }} Argument{{ cand.arguments.length != 1 ? 's' : '' }}
                    </span>
                  </td>
                  <td>
    ...
    autoCycling() {
      if (this.task) {
        ...
        if (clazz == "Plan") {
          ...
          return this.task.tasks.filter((subtask) => !subtask.optional && !subtask.autonomous).length == 0;
        } else {
    ...
    dataDefinitions() {
      if (this.task) {
        if (this.dataDefinitionsFilter) {
          return this.task.dataDefinitions.filter((dd) => dd.name.includes(this.dataDefinitionsFilter));
        } else {
  ...
  methods: {
    ...
    handleDDText(evt) {
      ...
      if (this.dataDefinitions.length > 0) {
        ...
        let def = new Protocol.Integer({ name: evt.target.value, caption: evt.target.value.replace(/([A-Z])/g, ' $1').replace(/^./, function (str) { return str.toUpperCase(); }) });
    ...
    sourceDataDefs() {
      ...
      return this.task.allDataDefinitions().filter(function (dd) { return !dd.valueCondition; });
    },
    availableDataDefs() {
      // return all non-dynamic data definitions that are not already used by this enquiry
      let sources = this.task.sources;
      function used(dd) {
        return sources.find(function (src) { return src.type == dd.name; });
      }
      return this.sourceDataDefs().filter(function (dd) { return !used(dd); });
    },
    ...
    renderCondition(src) {
      if (src.requestCondition) {
        ...
        if (mtch && mtch[1] == src.type) {
          mtch = src.requestCondition.match(/\|\|\s*last_updated\(\'(\w+)\'\)\.diff\(now\(\),\s*\'(minutes|hours|days|months|years)\'\)>(\d+)/)
          if (mtch && mtch[1] == src.type) {
    ...
    addCandidate(evt) {
      let cand = new Protocol.Candidate({ name: evt.target.value, caption: evt.target.value.replace(/([A-Z])/g, ' $1').replace(/^./, function (str) { return str.toUpperCase(); }), recommendCondition: "net_support('" + evt.target.value + "')>0" });
