...
<template>
    <div>
        <debug title="ComposeArgument">
            <ComposeArgument :protocol="protocol" :path="argumentPath" @change-protocol="$emit('change-protocol', $event)"
                @select-path="updateArgumentPath" ref="argumentEditor" :issues="subComponentIssues(argumentPath)" />
        </debug>
    </div>
</template >
