...
<template>
  <g class="arrow" :data-source="source" :data-target="target"
    @dblclick="$emit('delete-arrow', { source: source, target: target })" v-hammer:tap="handleDoubleTap">
...
export default {
  ...
  computed: {
    ...
    headPath: function () {
      ...
      let a1 = { x: this.tx + this.arrowsize * Math.cos(alpha), y: this.ty + this.arrowsize * Math.sin(alpha) };
      let a2 = { x: this.tx + this.arrowsize * Math.cos(beta), y: this.ty + this.arrowsize * Math.sin(beta) };
      return "M " + this.tx + " " + this.ty + " L " + a1.x + " " + a1.y + " L " + a2.x + " " + a2.y + " L " + this.tx + " " + this.ty;
    },
    arrowColor: function () {
