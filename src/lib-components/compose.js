  ...
  methods: {
    ...
    addAntecedent(planPath, source, target) {
      ...
      if (currentDeps.length==0) {
        ...
        this.replaceClause(targetTask, "waitCondition", "is_finished('" + currentDeps[0] + "')", "num_finished(['" + currentDeps[0] + "','" + source + "'])==2");
        this.replaceClause(targetTask, "preCondition", "is_completed('" + currentDeps[0] + "')", "num_completed(['" + currentDeps[0] + "','" + source + "'])>0");
      } else {
        ...
        this.replaceClause(targetTask, 'waitCondition', "num_finished(" + oldDeps + ")==" + (currentDeps.length-1), "num_finished(" + newDeps + ")==" + currentDeps.length);
        this.replaceClause(targetTask, 'preCondition', "num_completed(" + oldDeps + ")>0", "num_completed(" + newDeps + ")>0");
      }
    },
    removeAntecedent(planPath, source, target) {
      ...
      let currentDeps = this.getTemporalDeps(targetTask);
      if (currentDeps.length==0) {
        console.log('weird. attempting to remove a task antecedent,', source, 'from a task which has none', target);
      } else if (currentDeps.length==1) {
        ...
        this.replaceClause(targetTask, "waitCondition", "num_finished(" + oldDeps + ")==2", "is_finished('" + currentDeps[0] + "')");
        this.replaceClause(targetTask, "preCondition", "num_completed(" + oldDeps + ")>0", "is_completed('" + currentDeps[0] + "')");
      } else {
        let oldDeps = this.writeDepArray(currentDeps);
        currentDeps.splice(currentDeps.indexOf(source),1);
        let newDeps = this.writeDepArray(currentDeps);
    ...
    getExpressions(expr) {
      if (expr) {
        ...
        if (first.startsWith('is_finished') || first.startsWith('num_finished') || first.startsWith('is_completed') || first.startsWith('num_completed')) {
