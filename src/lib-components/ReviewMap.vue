  ...
  <svg ref="svg" width="100%" tabindex="0" :viewBox="viewbox">
    <template v-if="plan.tasks">
      <pm-breadcrumb v-if="plan.tasks" :plan="plan" :review="true" :height="breadcrumb.height"
        :width="protocol.meta.svg.width" :offset="0" @select-task="updatePlan" :selected="selectedplan === selectedtask" />
...
export default {
  ...
  computed: {
    ...
    plan() {
      ...
      try {
        ...
        let designpath = this.selectedplan.split(":").map((task) => task.indexOf("[") > -1 ? task.substring(0, task.indexOf("[")) : task).join(":");
    ...
    viewbox() {
      if (this.protocol && this.protocol.meta && this.protocol.meta.svg) {
        return "0 0 " + (this.protocol.meta.svg.width) + " " + (this.protocol.meta.svg.height + this.breadcrumb.height);
      }
    },
    ports() {
      let result = {};
      for (let task of this.plan.tasks) {
        result[task.name] = { to: { x: task.meta.pos.x, y: task.meta.pos.y + this.breadcrumb.height } };
        result[task.name].from = { x: task.meta.pos.x + 40, y: task.meta.pos.y + this.breadcrumb.height }
        // small adjustments
  ...
  methods: {
    ...
    runtimeTasks(plan) {
      return plan.tasks.map((task) => this.enactment.getComponent(this.enactment.runtimeFromDesignPath(task.path())));
    }
  },
  watch: {
