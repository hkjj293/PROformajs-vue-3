<docs>
Provides a proformajs Enactment object for Review. Upgradeing to vue3
      ...
      if (this.protocol && this.protocol.isValid()) {
        try {
          ...
          if (this.initialData && Object.keys(this.initialData).length > 0) {
            local = new Enactment({ start: false, protocol: this.protocol, options: this.enactmentOptions });
            local.setData(this.initialData);
          } else {
            local = new Enactment({ start: true, protocol: this.protocol, options: this.enactmentOptions });
          }
